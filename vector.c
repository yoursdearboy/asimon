/*
 * Vector implementation by Peter Goldsborough + cat
...
int vector_combine(Vector* destination, Vector* source1, Vector* source2) {
    ...
    assert(source1 != NULL);
    assert(source2 != NULL);
    assert(vector_is_initialized(source1));
    assert(vector_is_initialized(source2));
    assert(!vector_is_initialized(destination));
    assert(source1->element_size == source2->element_size);

    if (destination == NULL) return VECTOR_ERROR;
    if (source1 == NULL) return VECTOR_ERROR;
    if (source2 == NULL) return VECTOR_ERROR;
    if (source1->element_size != source2->element_size) return VECTOR_ERROR;

    destination->size = source1->size + source2->size;
    destination->capacity = (source1->size + source2->size) * 2;
    destination->element_size = source1->element_size;

    destination->data = malloc(destination->capacity * destination->element_size);
    if (destination->data == NULL) return VECTOR_ERROR;

    memcpy(destination->data, source1->data, vector_byte_size(source1));
    memcpy(destination->data + vector_byte_size(source1), source2->data, vector_byte_size(source2));
